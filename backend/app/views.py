    ...
    for post in posts:
        post_data = {
            ...
            "author": {
                ...
                "profile_image": post.author.profile_image.url if post.author.profile_image else ""
            },
            "post": {
                "title": post.title,
                "content": post.description,
                "tags": [tag.name for tag in post.tags.all()],
                "timestamp": post.created_at.strftime("%I:%M %p")  # Format as needed
            },
            "engagement": {
                "likes": post.likes.count(),  # If likes are tracked as a relationship or field
                "comments": post.comments.count()  # Assumes related name `comments` for Comment model
            },
            "comments": []
        }

        # Fetch and format comments for the post
        ...
        for comment in comments:
            post_data["comments"].append({
                ...
                "timestamp": comment.created_at.strftime("%I:%M %p"),  # Format as needed
                "likes": comment.likes.count()  # If likes are tracked as a relationship or field
            })
